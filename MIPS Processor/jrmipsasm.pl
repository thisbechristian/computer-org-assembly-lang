...
%mnemonic = (ADD  => 0x0000, SUB  => 0x0001, ADDI => 0x1000, ADDUI => 0x1100,
   	     NOR  => 0x3000, AND  => 0x3001, 
             SLLV  => 0x4000, SRLV  => 0x4001, SLL => 0x5000, SRL => 0x5001, 
             LW   => 0x6000, SW   => 0x6001,  
             HALT => 0x7000, 
             BZ   => 0x8000, BX => 0x9000, BP   => 0xA000, BN  => 0xB000, 
             JAL  => 0xC000, JR   => 0xD000, J => 0xE000, PUT  => 0xF000,
	     SET => 0x2000, SETR => 0x2100);
...
foreach (@source) {
	...
	emit($addr++,$mnemonic{PUT}+($regs{$rsyn{$1}}<<9)+($2 & 0xff), $line);
    }
    elsif (/^\s*PUT\s+\$([RT][0-7])\s*/) {
	emit($addr++,$mnemonic{PUT}+($regs{$rsyn{$1}}<<9)+(0 & 0xff), $line);
    }
	...
	emit($addr++,$mnemonic{$1}+($regs{$rsyn{$2}}<<9)+($3 & 0xff), $line);
    }
	...
	emit($addr++,$mnemonic{$1}+($regs{$rsyn{$2}}<<9)+($dlabel{$3} & 0xff), $line);
    }
	...
	emit($addr++,$mnemonic{$1}+($regs{$rsyn{$2}}<<9)+($label{$3} & 0xff), $line);
    }
	...
	if (!defined $options{p}) {
	    ...
	    emit($addr++,$mnemonic{ADDUI}+($regs{$rsyn{$1}}<<9)+($dlabel{$2} & 0xff), "");	    
	}
	else {
	...
	if (!defined $options{p}) {
	    ...
	    emit($addr++,$mnemonic{SUB}+($regs{$rsyn{$1}}<<9)+($regs{$rsyn{$1}}<<6), $line);
	    # load the immediate
	    emit($addr++,$mnemonic{ADDUI}+($regs{$rsyn{$1}}<<9)+($2 & 0xff), "");	    
	}
	else {
	...
	emit($addr++,$mnemonic{JAL}+($regs{$rsyn{$1}}<<9)+($label{$2} & 0xff), $line);
    }
    elsif (/^\s*JR\s+\$([RT][0-7])/) {
	emit($addr++,$mnemonic{JR}+($regs{$rsyn{$1}}<<9), $line);
    }
    elsif (/^\s*J\s+(\w+)/) {
	emit($addr++,$mnemonic{J}+($label{$1} & 0xff), $line);
    }
    elsif (/^\s*[\#;]?\s+/) {
